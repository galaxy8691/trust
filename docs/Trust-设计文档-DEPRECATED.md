...
- **Fuzzing 安全：** AI 或 fuzzer 生成的随机输入可以安全地注入 Trust 程序——即使触发极端路径，编译器保证不会出现段错误或数据竞争（详见 §5.5）。
- **逻辑竞态是人类审查的最后边界：** 编译通过不等于无 bug——逻辑竞态（race condition）仍需审查。Trust 通过并发压力测试框架（§14.4）辅助暴露此类问题。
