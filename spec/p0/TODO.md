...
- [ ] 注释格式：`//` 行注释、`/* */` 块注释、`///` 文档注释
- [ ] 运算符和分隔符完整列表
- [ ] **运算符优先级与结合性表**（含 `await` 中缀、`?.` 链式、`??` 空值合并、`!` 后缀断言、`!` 前缀逻辑非、`&` 引用、`=>` 箭头、`as` 转换等）
- [ ] **显式类型转换语法**：`expr as T`

### 0.1.2 语法规范

- [ ] 形式化 EBNF 语法定义（或类似形式化格式）
- [ ] **Parser 错误恢复策略与同步点定义**（panic mode，同步点：`;` `}` `function` `import` `export` `type` `interface` `impl` `test` `async`）
- [ ] 变量声明：`let` / `let mut` / `const` / **`shared`**（独立条目）
...
- [ ] FFI：`extern "rust" { fn ... }`（含 `fn` 语法、`...args` 变长、泛型参数细节）
- [ ] 属性：`#[test]` / `#[should_panic]` / `#[property]` / `#[concurrent]`
- [ ] 生命周期标注：`<'a>`（返回引用、返回含引用结构、关联参数与返回值生存期时使用；绝大多数场景自动推导）
- [ ] 引用：`&` 运算符
- [ ] 测试：`test function` / `#[test]`
- [ ] **`::` 构造器/关联函数访问**：`Type::ident`（`Box::new`、`Dynamic.Number`、`Vec::new` 等）
- [ ] **分号与分隔符规则**：语句 `;` 结尾；`match` 分支 `,` 分隔（末尾可选）；`switch` 分支内 `;` + `break;`；块内最后表达式省略 `;` 作为返回值

### 0.1.3 语义规范

- [ ] **AST 节点完整定义**（parser 输出，含 `if`/`loop`/`match` 表达式 vs `switch`/`for`/`while` 语句区分、`break` 带返回值仅限 `loop`）
- [ ] **HIR 节点完整定义**（符号表、作用域结构、名称解析）
- [ ] **AST → HIR 降级规则**（名称解析、模块路径解析、`import`/`export` 语义）
- [ ] **HIR 层类型检查与推断规则**（名义类型检查、结构别名等价、闭包参数推断、单表达式返回值推断、泛型参数调用点推断）
- [ ] **HIR → TIR 降级规则**（控制流图、基本块转换、表达式→语句转换）
...
- [ ] 结构别名（`type { ... }`）透明等价规则
- [ ] ADT（`type | ...`）标签联合规则
- [ ] **数字类型规则**：`number` 单一类型，整数默认 `i32`，浮点默认 `f64`，**禁止混合运算**，用 `as` 显式转换
- [ ] 泛型单态化规则
- [ ] trait bound（`extends Interface` vs `extends { ... }` 结构化约束）
- [ ] 隐式 trait 生成规则（`HasLength` 等）
- [ ] `this: &Self` / `inout this` / `move this` 隐式参数规则
- [ ] **`Dynamic` 枚举类型规则**（栈分配、变体构造、模式匹配穷举）
- [ ] **`Box<dyn Trait>` 类型规则**（vtable 分发、与泛型单态化区分场景）
- [ ] **`??` 运算符类型规则**（`Option<T> ?? T → T`、`Result<T,E> ?? T → T`，映射 `unwrap_or`）
- [ ] **`?.` 运算符类型规则**（`Option` 字段时 `and_then` vs 非 `Option` 字段时 `map` 自动选择；owned 上下文 move 语义）
- [ ] **`Send` / `Sync` 的自动推导类型规则**（基于字段组成的类型推导）
...
- [ ] **不可变绑定的方法调用限制**：`let`（非 `mut`）变量只能调用 `&self` 方法
- [ ] **可变绑定的方法调用规则**：`let mut` 可调用所有方法，调用期间独占借用
- [ ] 闭包捕获规则：默认只读借用，`move` → FnOnce
- [ ] **引用计数所有权规则**：`Rc::new`/`Arc::new` 创建；`clone()` 增引用；`Weak::upgrade`；`Rc<T>` 非 `Send` 不能跨线程
- [ ] 区域推断（Region Inference）算法伪代码
- [ ] `Copy` 类型判定规则
- [ ] **`for (let i = 0; ...; i++)` 隐式可变例外**：`i` 允许 `i++`，不可变默认的唯一例外

### 0.1.6 并发规则形式化

- [ ] **`Send` / `Sync` 的并发使用侧检查规则**：`spawn` 要求 `move` + `Send`；`shared` 要求 `Sync`；`Channel<T>` 要求 `T: Send`
...
- [ ] `std::async` API 签名（`join` `sleep`、异步 I/O 原语、Tokio 绑定）
...
- [ ] `std::process` API 签名（子进程管理、环境变量）— v0.2 目标
