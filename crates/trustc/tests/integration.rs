...
macro_rules! assert_output {
    ($fixture:expr, $expected:expr) => {{
        ...
        let run =
            Command::new(&format!("/tmp/trust_test_{}", name)).output().expect("binary should run");
...
#[test]
fn e2e_const_decl() {
    assert_output!("const_decl.trust", "const ok");
}
#[test]
fn e2e_for_loop() {
    assert_output!("for_loop.trust", "ok");
}
#[test]
fn e2e_if_else() {
    assert_output!("if_else.trust", "if branch");
}
#[test]
fn e2e_let_variable() {
    assert_output!("let_variable.trust", "let ok");
}
#[test]
fn e2e_let_mut() {
    assert_output!("let_mut.trust", "mut ok");
}
#[test]
fn e2e_logical() {
...
#[test]
fn e2e_ref_operator() {
    assert_output!("ref_operator.trust", "ref ok");
}
#[test]
fn e2e_return_value() {
    assert_output!("return_value.trust", "return ok");
}
#[test]
fn e2e_shared_decl() {
    assert_output!("shared_decl.trust", "shared ok");
}
#[test]
fn e2e_template_literal() {
    assert_output!("template_literal.trust", "template ok");
}
#[test]
fn e2e_while_loop() {
    assert_output!("while_loop.trust", "while ok");
}
...
#[test]
fn e2e_arrow_fn() {
    assert_compiles!("arrow_fn.trust");
}
#[test]
fn e2e_as_cast() {
...
fn e2e_function_call() {
    assert_compiles!("function_call.trust");
}
#[test]
fn e2e_import_export() {
    assert_compiles!("import_export.trust");
}
#[test]
fn e2e_nullish_coalesce() {
    assert_compiles!("nullish_coalesce.trust");
}
#[test]
fn e2e_param_inout() {
    assert_compiles!("param_inout.trust");
}
#[test]
fn e2e_type_annotation() {
    assert_compiles!("type_annotation.trust");
}

// === 覆盖率增强: 错误路径 + 复杂场景 ===
#[test]
fn e2e_multi_function() {
    assert_output!("multi_function.trust", "multi fn ok");
}
#[test]
fn e2e_nested_if() {
    assert_output!("nested_if.trust", "nested ok");
}
...
fn e2e_err_type_mismatch() {
    let out = run_trustc(&[
        "compile",
        "tests/fixtures/err_type_mismatch.trust",
        "-o",
        "/tmp/trust_err_type",
    ])
    .unwrap();
...
fn e2e_err_undefined_var() {
    let out = run_trustc(&[
        "compile",
        "tests/fixtures/err_undefined_var.trust",
        "-o",
        "/tmp/trust_err_undef",
    ])
    .unwrap();
...
fn e2e_err_param_count() {
    let out = run_trustc(&[
        "compile",
        "tests/fixtures/err_param_count.trust",
        "-o",
        "/tmp/trust_err_param",
    ])
    .unwrap();
...
fn e2e_err_move_use() {
    let out =
        run_trustc(&["compile", "tests/fixtures/err_move_use.trust", "-o", "/tmp/trust_err_move"])
            .unwrap();
...
#[test]
fn e2e_if_else_both() {
    assert_output!("if_else_both.trust", "positive");
}
#[test]
fn e2e_shadow_var() {
    assert_output!("shadow_var.trust", "ok");
}
#[test]
fn e2e_err_return_type() {
    let out = run_trustc(&[
        "compile",
        "tests/fixtures/err_return_type.trust",
        "-o",
        "/tmp/trust_err_ret",
    ])
    .unwrap();
...
fn e2e_err_binary_type() {
    let out = run_trustc(&[
        "compile",
        "tests/fixtures/err_binary_type.trust",
        "-o",
        "/tmp/trust_err_bin",
    ])
    .unwrap();
...
fn e2e_err_duplicate_var() {
    let out = run_trustc(&[
        "compile",
        "tests/fixtures/err_duplicate_var.trust",
        "-o",
        "/tmp/trust_err_dup",
    ])
    .unwrap();
...
#[test]
fn e2e_cmp_all() {
    assert_output!("cmp_all.trust", "cmp ok");
}
#[test]
fn e2e_cast_combo() {
    assert_output!("cast_combo.trust", "cast ok");
}
...
fn e2e_err_const_type() {
    let out = run_trustc(&[
        "compile",
        "tests/fixtures/err_const_type.trust",
        "-o",
        "/tmp/trust_err_const",
    ])
    .unwrap();
...
fn e2e_err_not_nonbool() {
    let out = run_trustc(&[
        "compile",
        "tests/fixtures/err_not_nonbool.trust",
        "-o",
        "/tmp/trust_err_not",
    ])
    .unwrap();
...
fn e2e_err_neg_nonnum() {
    let out =
        run_trustc(&["compile", "tests/fixtures/err_neg_nonnum.trust", "-o", "/tmp/trust_err_neg"])
            .unwrap();
...
fn e2e_err_call_arg_type() {
    let out = run_trustc(&[
        "compile",
        "tests/fixtures/err_call_arg_type.trust",
        "-o",
        "/tmp/trust_err_callarg",
    ])
    .unwrap();
...
#[test]
fn e2e_mut_assign() {
    assert_output!("mut_assign.trust", "mut assign ok");
}
#[test]
fn e2e_err_immut_assign() {
    let out = run_trustc(&[
        "compile",
        "tests/fixtures/err_immut_assign.trust",
        "-o",
        "/tmp/trust_err_im",
    ])
    .unwrap();
...
mod cli_tests {
    ...
    fn cli_no_command() {
        ...
        assert!(!out.status.success());
        assert!(
            String::from_utf8_lossy(&out.stderr).contains("no command")
                || String::from_utf8_lossy(&out.stderr).contains("Usage")
        );
    }
    #[test]
    ...
    #[test]
    fn cli_output_flag() {
        let _ = run_trustc(&["compile", "tests/fixtures/hello.trust", "-o", "/tmp/trust_cli_o"])
            .unwrap();
    }
}
