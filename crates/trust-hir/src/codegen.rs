...
pub fn emit_rust_with_options(
    ...
    if module_needs_fetch_text_helper(module) {
        out.push_str(
            "async fn __trust_fetch_text(url: String) -> String {\n    reqwest::get(url.as_str())\n        .await\n        .expect(\"HTTP GET failed\")\n        .text()\n        .await\n        .expect(\"read response body failed\")\n}\n\n",
        );
    }
    if module_needs_fetch_request_helper(module) {
        out.push_str(
            r#"#[derive(Clone)]
struct __TrustFetchInit {
    method: String,
    headers: Vec<(String, String)>,
    body: Option<String>,
}

async fn __trust_fetch(url: String, init: Option<__TrustFetchInit>) -> reqwest::Response {
...
"#,
    ...
    if module_needs_utf16_stdlib_helpers(module) {
        out.push_str(
            r#"fn __trust_utf16_slice(s: &str, start: i32, end: Option<i32>, substring_swap: bool) -> String {
...
fn __trust_utf16_index_of(haystack: &str, needle: &str, from_utf16: i32) -> f64 {
...
"#,
    ...
    if module_needs_json_stringify_helper(module) {
        out.push_str(
            r#"fn __trust_json_escape_string(s: &str) -> String {
...
fn emit_expr(
    ...
    match e {
        ...
        IRExpr::JsonBuiltin {
            ...
            JsonBuiltinKind::Stringify => {
                ...
                if is_stringish(ty) {
                    Ok(format!("__trust_json_escape_string(&({inner}))"))
    ...
    __line
        ...
        IRExpr::FetchText { url, .. } => {
            ...
            Ok(format!("__trust_fetch_text({u})"))
        }
...
fn emit_fetch_call(
    ...
    if !fetch_init_needs_some(init) {
        return Ok(format!("__trust_fetch({u}, std::option::Option::None)"));
    }
    ...
    Ok(format!(
        "__trust_fetch({u}, std::option::Option::Some(__TrustFetchInit {{ method: {:?}.to_string(), headers: vec![{headers_joined}], body: {body_rust} }}))",
        method
    ))
}
...
fn emit_string_method_builtin(
    ...
    match kind {
        ...
        Slice => {
            ...
            if args.len() == 1 {
                Ok(format!(
                    "__trust_utf16_slice(&({recv}), ({a0}) as i32, None, false)"
                ))
            } else {
                let a1 = emit_expr(&args[1], f, stmt_level, module)?;
                Ok(format!(
                    "__trust_utf16_slice(&({recv}), ({a0}) as i32, Some(({a1}) as i32), false)"
                ))
            }
        }
        Substring => {
            ...
            if args.len() == 1 {
                Ok(format!(
                    "__trust_utf16_slice(&({recv}), ({a0}) as i32, None, true)"
                ))
            } else {
                let a1 = emit_expr(&args[1], f, stmt_level, module)?;
                Ok(format!(
                    "__trust_utf16_slice(&({recv}), ({a0}) as i32, Some(({a1}) as i32), true)"
                ))
            }
        }
        IndexOf => {
            ...
            Ok(format!(
                "__trust_utf16_index_of(&({recv}), &({needle}), ({from}) as i32)"
            ))
        }
        Includes => {
            let needle = emit_expr(&args[0], f, stmt_level, module)?;
            ...
            Ok(format!(
                "(__trust_utf16_index_of(&({recv}), &({needle}), ({from}) as i32) >= 0.0)"
            ))
        }
    }
}
...
mod tests {
  ...
  return "ab".slice(0, 1).length + JSON.stringify(1).length;
        ...
        check_module(&mut m).unwrap();
        let rs = emit_rust_with_options(&m, &CodegenOptions::default()).unwrap();
        assert!(rs.contains("__trust_utf16_slice"), "{rs}");
        assert!(rs.contains("__trust_json_escape_string"), "{rs}");
    }
