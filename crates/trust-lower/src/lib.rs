...
use thiserror::Error;
use trust_hir::{CodegenOptions, CompileError, CompileWarning, TrustManifest};
...
pub fn lower_program_with_options(
    ...
    Ok(trust_hir::compile_with_options(program, cm, path, codegen)?)
}

/// 多文件模块图：与 [`trust_hir::compile_graph`] 等价，供 driver/CLI 主路径调用。
...
pub fn lower_module_graph_with_options(
    ...
    Ok(trust_hir::compile_graph_with_options(
...
pub fn check_module_graph(
    ...
    Ok(trust_hir::check_graph_with_trust(units, entry_path, trust)?)
}
...
mod tests {
    ...
    use trust_parser::{parse_module_graph, validate_imports};
...
"#;
        ...
        let opts = trust_hir::CodegenOptions {
            span_comments: true,
            ..Default::default()
        };
