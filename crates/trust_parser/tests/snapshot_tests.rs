...
#[test] fn snap_fn_move() { assert_ast_snapshot("function consume(move data:number[]){}", &["FunctionDecl", "Move"]); }
#[test] fn snap_fn_single() { assert_ast_snapshot("function sq(x:number)=x*x", &["FunctionDecl", "sq"]); }
#[test] fn snap_if() { assert_ast_snapshot("if(x>0){1}else{0}", &["IfExpr", "Binary"]); }
#[test] fn snap_for_c() { assert_ast_snapshot("for(let i=0;i<10;i=i+1){}", &["ForStmt", "LetStmt", "i"]); }
#[test] fn snap_for_of() { assert_ast_snapshot("for(let item of items){}", &["ForOfStmt", "item"]); }
#[test] fn snap_while() { assert_ast_snapshot("while(x>0){x=x-1}", &["WhileStmt", "Binary"]); }
#[test] fn snap_loop() {
    assert_ast_snapshot("loop{if(x>3){break x*2}}", &["LoopExpr", "BreakStmt"]);
}
#[test] fn snap_return() { assert_ast_snapshot("return 42", &["ReturnStmt"]); }
#[test] fn snap_break() { assert_ast_snapshot("break", &["BreakStmt"]); }
#[test] fn snap_continue() { assert_ast_snapshot("continue", &["ContinueStmt"]); }
#[test] fn snap_ref() { assert_ast_snapshot("let r=&data", &["LetStmt", "Reference"]); }
#[test] fn snap_bang() { assert_ast_snapshot("let v=maybeValue!", &["LetStmt", "AssertUnwrap"]); }
#[test] fn snap_try() { assert_ast_snapshot("let f=open(\"a.txt\")?", &["LetStmt", "TryPropagate"]); }
#[test] fn snap_nullish() { assert_ast_snapshot("let n=maybeName??\"anon\"", &["QuestionQuestion"]); }
#[test] fn snap_optchain() { assert_ast_snapshot("let s=user?.addr?.street", &["MemberAccess", "optional: true"]); }
#[test] fn snap_import_named() { assert_ast_snapshot("import{foo,bar}from\"./util\"", &["ImportDecl", "Named", "foo", "bar"]); }
#[test] fn snap_import_default() { assert_ast_snapshot("import g from\"./g\"", &["ImportDecl", "Default", "g"]); }
#[test] fn snap_import_ns() { assert_ast_snapshot("import*as m from\"./m\"", &["ImportDecl", "Namespace", "m"]); }
#[test] fn snap_export() { assert_ast_snapshot("export function f(){}", &["ExportDecl", "FunctionDecl"]); }
#[test] fn snap_arrow() { assert_ast_snapshot("let f=(x)=>x*2", &["ArrowFn", "x"]); }
#[test] fn snap_move_closure() { assert_ast_snapshot("let c=move()=>process()", &["ArrowFn", "is_move: true"]); }
#[test] fn snap_block_ret() { assert_ast_snapshot("let x={let y=2;y}", &["BlockExpr"]); }
#[test] fn snap_as_cast() { assert_ast_snapshot("let c=a as f64+b", &["AsCast"]); }
#[test] fn snap_type_ann() { assert_ast_snapshot("let x:number=42", &["NumberType"]); }
#[test] fn snap_newline_sep() {
...
#[test] fn snap_error_panic_mode() {
    ...
    assert!(prog.statements.len() >= 1, "expected >=1 recovered stmt (x), got {}", prog.statements.len());
}
