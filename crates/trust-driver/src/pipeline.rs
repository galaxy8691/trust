...
pub(crate) fn compile_entrypoint_to_executable_impl(
    ...
    let trust = match trust_manifest::discover_trust_toml(path) {
        Some(p) => Some(trust_manifest::TrustManifest::load(&p)?),
        None => None,
    };
    let graph = trust_parser::parse_module_graph_with_trust(path, &[], trust.as_ref())?;
    trust_parser::validate_imports(&graph)?;
    let units = graph.compile_units();
    let entry_path = graph.entry_path_str();
    let codegen = trust_hir::CodegenOptions::default();
    let (rust, warnings) = trust_lower::lower_module_graph_with_options(
        &units,
        &entry_path,
        trust.as_ref(),
        &codegen,
