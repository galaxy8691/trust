...
const CRATE_NAME: &str = "trust_generated";
...
pub enum DriverError {
    ...
    #[error(transparent)]
    Parse(#[from] trust_parser::ParseError),

    #[error(transparent)]
    Lower(#[from] trust_lower::LowerError),
...
/// `target/release/trust_generated`（Windows 下为 `.exe`）。详见 [crate 级说明](crate#临时目录与-tempdir)。
...
/// **临时目录句柄**（保活目录）与 **`target/release/trust_generated`** 可执行路径（Windows 带 `.exe`）。
///
...
/// Same as [`build_rust_to_executable`], with [`RustBuildOptions`] (e.g. optional `trust_rt` in `Cargo.toml`).
...
mod tests {
    ...
    fn write_minimal_crate_with_link_trust_rt_contains_optional_path_dep() {
        ...
        assert!(
            toml.contains("trust_rt") && toml.contains("optional = true"),
            "unexpected Cargo.toml:\n{toml}"
        );
        assert!(toml.contains("[features]") && toml.contains("dep:trust_rt"));
    }
    ...
    fn map_cargo_spawn_error_maps_not_found_to_cargo_not_found() {
        let io_err = Command::new("this_binary_must_not_exist_trust_6_1")
            .output()
