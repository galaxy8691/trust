...
| `interface` `extends`, imported interface / type names **across dependency modules**, `import type` / type-only import specifiers | **`interface extends`** (single inheritance) with width subtyping implemented; **`import type { I } from "./dep.ts"`** across files works; nominal methods via `obj.method()` supported; see matrix `interface` row (**not** full `tsc` structural rules). |
...
| Member access | Partial | Deep chains `f().g().h()`; `string.length` UTF-16 code units; `string[i]` UTF-16 index; `number[].length`; `obj.m(args)` → global `m(receiver,…)` (nominal methods); interface method calls; **chained** `f().prop` / `f().m()`; no computed `obj[expr](…)`; `chain_call_ok.ts`, `deep_member_call_ok.ts`, `interface_method_call_ok.ts`, `string_utf16_length.ts`, `method_call_ok.ts` |
...
| `interface` | Partial | Top-level with **`extends`** inheritance (single, width subtyping); nested/optional props; **`import type { I } from "./dep.ts"`** across files works; nominal methods `obj.method()` supported via `m__obj` pattern; **no** callable/`( ) =>` members in object types; `interface_ok.ts`, `interface_extends_ok.ts`, `interface_method_call_ok.ts`, `import_type_ok_main.ts`, negatives |
